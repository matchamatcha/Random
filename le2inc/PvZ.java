...
public class PvZ {
    ...
    public void simulateGame() {
        ...
        List <Producer> p = getProducerPlants();
        for(Producer pp :p)
        sunlight += pp.produce();
    ...
    public List<Producer> getProducerPlants() {
        ...
        for (Character[] col : lawn) {
            ...
            for (Character chr : col) {
                ...
                //Producer producers = new Sunflower(producers);
    ...
    public void displayLawn() {
        ...
        try {
            ...
            File file = new File("C:\\Users\\ADMIN\\Desktop\\cmsc22midterm\\Desktop\\cmsc22midterm\\out.txt");
            // if file doesnt exists, then create it
            if (!file.exists()) {
                file.createNewFile();
            }

            BufferedWriter b = new BufferedWriter(new FileWriter(file.getAbsoluteFile(),true));
        ...
        for (int i = 0; i < lawn.length; i++) {
            for (int j = 0; j < lawn[i].length; j++) {
                if (lawn[i][j] == null) {
                    ...
                    b.write(lawn[i][j].getClass().getSimpleName().toLowerCase().toString().charAt(0) + " ");
                } else if (lawn[i][j] instanceof Plant) {
                    // gets first character of class name and prints it out...
                    b.write(lawn[i][j].getClass().getSimpleName().toString().charAt(0) + " ");
                }
            }
            b.newLine();
        }
        
        b.write("sunlight: " + sunlight);
        b.newLine();
